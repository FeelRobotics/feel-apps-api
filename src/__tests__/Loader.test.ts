...
describe('Loader.loadSubtitlesInfo — URL building', () => {
  it('always includes apptoken', async () => {
    mockFetch.mockResolvedValue(ok({ session_id: '1', text: '{}' }));
    await Loader.loadSubtitlesInfo('vid', 1, null, '');
    expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining('apptoken=tok'));
  });

  it('parses subtitlesId string to integer in the path', async () => {
    mockFetch.mockResolvedValue(ok({ session_id: '1', text: '{}' }));
    await Loader.loadSubtitlesInfo('vid', '42.9', null, '');
    expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining('/subtitles/42'));
  });

  it('encodes videoId in the path', async () => {
    mockFetch.mockResolvedValue(ok({ session_id: '1', text: '{}' }));
    await Loader.loadSubtitlesInfo('my video/id', 1, null, '');
    expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining('my%20video%2Fid'));
  });

  it('adds external_user_id when provided', async () => {
    mockFetch.mockResolvedValue(ok({ session_id: '1', text: '{}' }));
    await Loader.loadSubtitlesInfo('vid', 1, 'user-99', '');
    expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining('external_user_id=user-99'));
  });

  it('omits external_user_id when null', async () => {
    mockFetch.mockResolvedValue(ok({ session_id: '1', text: '{}' }));
    await Loader.loadSubtitlesInfo('vid', 1, null, '');
    expect(mockFetch).toHaveBeenCalledWith(expect.not.stringContaining('external_user_id'));
  });

  it('adds channel when provided', async () => {
    mockFetch.mockResolvedValue(ok({ session_id: '1', text: '{}' }));
    await Loader.loadSubtitlesInfo('vid', 1, null, 'ch1');
    expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining('channel=ch1'));
  });

  it('omits channel when empty string', async () => {
    mockFetch.mockResolvedValue(ok({ session_id: '1', text: '{}' }));
    await Loader.loadSubtitlesInfo('vid', 1, null, '');
    expect(mockFetch).toHaveBeenCalledWith(expect.not.stringContaining('channel='));
  });
  ...
  it('adds pubnub_uuid when clientId is set', async () => {
    ...
    mockFetch.mockResolvedValue(ok({ session_id: '1', text: '{}' }));
    await Loader.loadSubtitlesInfo('vid', 1, null, '');
    expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining('pubnub_uuid=client-abc'));
  });
  ...
  it('omits pubnub_uuid when clientId is empty', async () => {
    ...
    mockFetch.mockResolvedValue(ok({ session_id: '1', text: '{}' }));
    await Loader.loadSubtitlesInfo('vid', 1, null, '');
    expect(mockFetch).toHaveBeenCalledWith(expect.not.stringContaining('pubnub_uuid'));
  });
});
