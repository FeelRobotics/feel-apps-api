...
describe('Loader.loadSubtitlesInfo — URL building', () => {
  ...
  it('parses subtitlesId string to integer in the path', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining('/subtitles/42'), expect.anything());
  });

  it('encodes videoId in the path', async () => {
    mockFetch.mockResolvedValue(ok({ session_id: '1', text: '{}' }));
    await Loader.loadSubtitlesInfo('my video/id', 1, null, '');
    expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining('my%20video%2Fid'), expect.anything());
  });

  it('adds external_user_id when provided', async () => {
    mockFetch.mockResolvedValue(ok({ session_id: '1', text: '{}' }));
    await Loader.loadSubtitlesInfo('vid', 1, 'user-99', '');
    expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining('external_user_id=user-99'), expect.anything());
  });

  it('omits external_user_id when null', async () => {
    mockFetch.mockResolvedValue(ok({ session_id: '1', text: '{}' }));
    await Loader.loadSubtitlesInfo('vid', 1, null, '');
    expect(mockFetch).toHaveBeenCalledWith(expect.not.stringContaining('external_user_id'), expect.anything());
  });

  it('adds channel when provided', async () => {
    mockFetch.mockResolvedValue(ok({ session_id: '1', text: '{}' }));
    await Loader.loadSubtitlesInfo('vid', 1, null, 'ch1');
    expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining('channel=ch1'), expect.anything());
  });

  it('omits channel when empty string', async () => {
    mockFetch.mockResolvedValue(ok({ session_id: '1', text: '{}' }));
    await Loader.loadSubtitlesInfo('vid', 1, null, '');
    expect(mockFetch).toHaveBeenCalledWith(expect.not.stringContaining('channel='), expect.anything());
  });
  ...
  it('adds pubnub_uuid when clientId is set', async () => {
    ...
    mockFetch.mockResolvedValue(ok({ session_id: '1', text: '{}' }));
    await Loader.loadSubtitlesInfo('vid', 1, null, '');
    expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining('pubnub_uuid=client-abc'), expect.anything());
  });
  ...
  it('omits pubnub_uuid when clientId is empty', async () => {
    ...
    mockFetch.mockResolvedValue(ok({ session_id: '1', text: '{}' }));
    await Loader.loadSubtitlesInfo('vid', 1, null, '');
    expect(mockFetch).toHaveBeenCalledWith(expect.not.stringContaining('pubnub_uuid'), expect.anything());
  });
});
