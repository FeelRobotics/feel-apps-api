...
function sendPlay(nextSubtitles: SubtitleEntry[], socket: Socket, roomId: string): void {
  const data = {
    room: roomId,
    src: socket.id ?? '',
...
export function play(
  ...
  const nextSubtitles = getNextSubtitles(subtitles, positionMsec, CHUNK_SIZE_MSEC)
